...
public class Packet {
	 ...
	 */
	public Packet(ChannelHandlerContext session, byte[] pData, boolean bare, int pid) {
		this.session = session;
		this.pData = pData;
		this.pLength = pData.length;
		this.buffer = ByteBuffer.wrap(pData);
	 ...
	 */
	public Packet(ChannelHandlerContext session, byte[] pData, int pid) {
		this(session, pData, false, pid);
	}
	...
	public String toString() {
		...
		String origin = session == null ? "this" : ((InetSocketAddress)session.getChannel().getLocalAddress()).getAddress().getHostAddress();
