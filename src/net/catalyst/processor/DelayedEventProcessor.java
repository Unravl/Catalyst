...
public abstract class DelayedEventProcessor implements Runnable {
	
	public DelayedEventProcessor(Client c, int ms, int loops) {
		this.setClient(c);
		this.setTime(ms);
		this.setLoops(loops);
	...
	public DelayedEventProcessor(Client c, int ms) {
		this(c, ms, 1);
	}
